...
局域网联机时，其他电脑需要打开主机电脑的局域网地址，例如：

```text
http://192.168.3.43:5173
```

不要在其他电脑上打开 `localhost` 地址；`localhost` 只代表当前电脑自己。更多说明见 [docs/online-mode.md](docs/online-mode.md)。
...
详细新手教程见 [docs/player-guide.md](docs/player-guide.md)。
积分和排行榜规则见 [docs/rating-system.md](docs/rating-system.md)。

1. 首页点击“开始游戏”，系统随机城邦并初始化 1 名玩家和 3 名 AI。
2. 每回合玩家看到 5 张商店牌，可以购买、刷新、锁定、整理或弃牌。
3. 手牌达到 14 张并满足胡牌结构时，结算会识别胡牌类型与胡牌战力分。
...
- 12 个原创海克斯。
- 5 个原创城邦。
- 规则评分 AI：购买高价值牌、必要时刷新、手牌超限弃牌。
- Smart AI 1.0：评估普通胡、七对子、碰碰胡、清一色路线，接近胡牌时更积极搜牌。
- 首页、游戏主界面、规则页、海克斯弹窗、结算弹窗。
- 胜利 / 死亡 / 最终排名全局弹窗。
- 操作音效和低音量无歌词 BGM，音效音量与 BGM 音量可分开调整。
- 单机积分、联机积分、段位和排行榜入口。
- 联机 MVP：
  - 创建 / 加入房间
  - 房间号复制
